...
| M5 前端（首页/结果页/对比/地图/修改/分享/OG 预览） | ✅ 完成（首页表单 → 三套方案时间线 + 逐项对比表 → **`/trip/{id}` 独立结果页**（刷新不丢）→ 改路线 / 撤销 / 分享 / 分享页动态 OG 图 + JSON-LD；地图需配一个 JS API Key） |
| M6 知识库扩量与质检 | ⏳ 待开始 |
...
| 分享路线 | ✅ 已可用：`/t/{slug}` 只读分享页（取消分享后旧链接立刻 404）；陌生人可以「复制成自己的一份」，改副本不影响原作者 | — |
| 方案对比（哪套走得少、哪套贵） | ✅ 已可用：方案卡片上方有一张逐项对比表（带未知/估算标记，只做对齐不排名；只有一套方案时不画表） | — |
| 结果页自己的 URL（`/trip/{id}`） | ✅ 已可用：刷新 / 收藏 / 复制地址都停在同一版；改路线与撤销会把地址换成新那一版（旧版本地址仍可打开） | — |
| 分享页的 OG 图 / JSON-LD | ✅ 已可用：`/t/{slug}` 带动态生成的 1200×630 预览图（微信/微博/Twitter 抓得到）与 `schema.org/TouristTrip` 结构化数据；失效链接画「不可用」空卡并 `noindex` | — |
