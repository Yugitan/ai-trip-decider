...
| M1-9 | 地点关系图 | 🟡 | `docs/RELATION_REPORT.json`：5445 对关系（800 地点），其中真实路网 **130 对 = 批内可覆盖的上限**（`osrm_batches_failed: 0`），其余为估算（逐条标注）。覆盖上限由 OSRM table 的**分批结构**决定，不是网络问题 —— 2026-09-16 复核，见"关系图报告的可归因性" |
...
| # | 事项 | 状态 | 原因与影响 |
| --- | --- | --- | --- |
| B1 | `place=*` 类目（街区/岛屿/村落）未抓取 | ✅ 已补（2026-09-15） | `fetch_osm_curated_extras.py` 定向补抓成功：212 条记录入库，`district` **8→13**（质检门槛 ≥10）、`citywalk` 1→2；二沙岛 / 东山口 / 小洲 / 花城广场 / 沙面街道 / 太古仓 / 琶醍 / 泮溪酒家 均已入库。仍缺（**不编造**）：永庆坊、上下九步行街、北京路步行街、广州城隍庙、海珠湖公园、粤海关博物馆 —— 这些**具体名称**在 OSM 无记录 |
| B2 | `transport_hub`（地铁站/公交/轮渡）未抓取 | ✅ 已补（2026-09-15） | 复测定位到根因**不是数据量，是查询写法**：正则过滤器 `["railway"~"^(station|subway_entrance)$"]` 迫使 Overpass 全量正则扫描 → 整块 bbox 稳定 504；换成逐值等值过滤后**同一 bbox 直接 200**。修复在 `fetch_osm_guangzhou.value_filters`（全组通用），补抓脚本 `fetch_osm_transport.py`：2708 条原始 → **落库 1525 条** `transport_hub`，"从哪开始"的起点建议因此可用 |
| B3 | `wikidata.org` / `zh.wikipedia.org` 不可达 | ⛔ | 无法用它们提升"身份可交叉核验"占比（当前 5.3%，长期目标 30%） |
| B4 | ~~真实路网关系覆盖仅约 2.4%~~ | ✅ 已解决（2026-09-16） | 根因不是网络、也不是 `batch_size`，而是**分批方式**：按 UUID 排序切连续块，而 UUID 是新生成的主键、与地理位置无关。改成按候选关系图做贪心聚类（`cluster_batches`）后，同一份 5445 对候选里真实路网对数 **130 → 4085（2.4% → 75.0%）**，32 批、0 批失败、28.3s，没有改接口也没有调 `batch_size`。剩下的 25% 是分批的固有代价（一批只能放 25 个点），要再往上只能自建 OSRM、减小 `batch_size`（请求数上升）或换商业地图服务 |
| B5 | 路线/门票价格来源 | 🟡 部分改善 | 全库 **3166 个地点里 0 条有 `price_min/price_max`** —— 这不是"预算留空"，而是"预算里全是零"：一条两顿老字号的路线会报出 `¥19.59/人`（只有一段打车费）。已改为**餐饮站点缺价时用 `config/limits.yaml` 的餐费单价推定**，并逐项写进新增的 `estimated_items`（与"没算进去"的 `unknown_items` 分开），前端同时显示两者。门票仍无来源，照旧留空不编 |
| B6 | ~~营业时间结构化解析~~ | ✅ 已解决（2026-09-16） | 解析器 `app/domain/opening_hours.py` 落地（TASKS.md 里登记为 M2 缺口的那个），在 `kb_layers.to_domain_place` **读时解析** `opening_hours_raw`：509 条原文解出 **267 条（52.5%）**，看不懂的（跨零点 `05:56-00:25`、脏数据 `06:06-24:08`）一律返回 `None` 并继续报 HOURS_UNKNOWN。**不落库到 `opening_hours` 列**：解析只有一份实现、原文才是权威，全量解析实测 1.4ms，不值得多一条会漂的副本 |
...
## M7 性能与安全检查（2026-09-16）

> M7 的第 3 项（E2E 16 场景与异常注入 25 项已在前两个提交里）。
> 这一轮把上一轮**没做完的收尾**补齐：压测脚本、前端 LCP 采集与报告都已经写出来了，
> 但（a）报告里写着"生成方式：`make perf`"而 **Makefile 里没有这个目标**，
> （b）`benchmark.py` 的文件头写着"与 `tests/integration/test_perf_budget.py` 共用分位函数"
> 而**那个文件不存在**。这次把这两处空缺补上，并让门槛真的被守住。
>
> 收尾之后又把报告里最后一个 `⏭ 不可测` 变成了真读数：**首页 TTI**（长任务分析），
> 修掉两处会让数字变假的缺陷（门槛量成机器负载、TTI 缺"在途请求"那一半）。

### 交付

| # | 任务 | 状态 | 证据 |
| --- | --- | --- | --- |
| M7-1 | `scripts/benchmark.py`：冷/缓存/修改/并发/束搜索/慢 SQL 全面压测 + 报告生成 | ✅ | 一条命令重跑；跨循环的引擎缓存问题已在脚本里绕开（所有阶段同一个事件循环） |
| M7-2 | `frontend/scripts/web-vitals.mjs`：Playwright + CDP 取 LCP | ✅ | 4G + CPU 4x + 移动端 viewport，取 3 次中位数；**不用 Lighthouse CLI**（不为一行指标再装一个浏览器） |
| M7-3 | `frontend/scripts/perf-lcp.sh`：可重复的采集编排 | ✅ | 复用已健康服务/自己起前后端 + 造**真实**分享行程拿 slug + 收尾只杀自己起的进程；造不出 slug 时不编造 |
| M7-4 | `make perf` / `perf-build` / `perf-lcp` / `perf-backend` | ✅ | 报告里那句"生成方式：`make perf`"从**一句空话**变成可执行的命令；`.perf/` 存原始证据并入库忽略 |
| M7-5 | `tests/integration/test_perf_budget.py`（秒级门槛回归） | ✅ | 17 条：门槛常量 ↔ PRD 互相钉住、分位口径、冷规划 p95 ≤ 8s、束搜索 ≤ 500ms（重复取最好）、**TTI 判定 7 条**、**报告不说假话**（3 条） |
| M7-6 | `tests/unit/test_db_session.py`（引擎缓存生命周期） | ✅ | 3 条：缓存必被清空、跨循环关闭失败不允许留下毒丸、其余异常不许吞 |
| M7-7 | 报告重新生成 | ✅ | `docs/PERF_REPORT.md`：分享页 LCP **392 ms** ✅ · 首屏 JS **127 KB** ✅ · **首页 TTI 569 ms** ✅ · 含搜索 **⏭ 不可测** |
| M7-8 | 首页 TTI 的长任务分析（采集 + 判定分开） | ✅ | 采集在 `web-vitals.mjs`（长任务 + Resource Timing 的在途区间，两半都收）；判定是 `benchmark.py::compute_tti_ms`（纯函数，7 条单测）。实测 3 次 [572, 569, 562] ms、各次只有 1 个长任务 → **569 ms ✅** |
| M7-9 | 分享页 TTI 作为**观察值**（PRD 未设门槛） | ✅ | 各次 [7984, 7737, 7714] ms、长任务 30–32 个 → 首页达标但结果页主线程阻塞明显，如实写进报告，不冒充门槛 |
| M7-10 | 束搜索门槛改成「重复 N 次取最好一次」 | ✅ | 见问题 84：门槛原本量的是**这台机器当时有多忙**；现在 `best_and_worst()` 是纯函数，报告里同时给出最差一次 |

### 本轮发现并修复的问题

| # | 问题 | 严重度 | 修法 |
| --- | --- | --- | --- |
| 76 | **"首屏 JS ≤ 200KB"这条门槛从来没有被算出来过**：`parse_build_log` 按 `token.endswith("kB")` 找体积，而 `next build` 打印的是"数字 + 单位"两个 token，命中的永远是那个光秃秃的 `kB` —— `float("kB"[:-2])` 抛 `ValueError` 后**返回 None**。于是首页与 shared 两项恒为 `None`，那一行从来不进报告、也从来不会失败（写这一条测试时才拓出来） | 🔴 高（门槛是死的，而且没人知道） | 改用正则 `(\d+(?:\.\d+)?)\s*kB` 取数；顺带取到 shared 的 102 kB 与全部 10 条路由 |
| 77 | **没测到的门槛被渲染成已达标的 ✅**（两处）：并发 20 那条 `target_ms` 为 `None`，而 `Sample.ok` 在没门槛时返回 `None`，渲染分支把它当"不是 False"处理 → 打 ✅（哪怕有 5xx 也一样）；首页 TTI 更是直接**拿 LCP 的读数**顶上门槛去打勾 | 🔴 高（诚实性：报告里最漂亮的那一行恰好是假的） | `Sample.auto_ok` 由测量方显式给结论（并发那条按"有无 5xx"判）；无门槛又无结论时渲染成 **`—（无门槛）`**；TTI 改成 `skipped=True` → **`⏭ 不可测`**，LCP 读数降级为观察值 |
| 78 | **`dispose_engines()` 先关池、后清缓存**：跨事件循环关闭异步引擎时 asyncpg 会抛 `got Future ... attached to a different loop`，而这一抛发生在 `clear()` **之前** —— 缓存里留下一个**已经不可用但还在的**引擎，后面每条用到它的用例都会再抛一次。实测是**间歇性**的（取决于那一刻池里有没有空闲连接），表现为新加的束搜索用例偶发失败 | 🟡 中（会变成一串间歇性失败） | 先摘缓存再关：最坏情况只是一个池子没被优雅关闭（进程退出时由 OS 回收）；只吞 `RuntimeError`（跨循环那一类），其余照旧抛，并用替身引擎把三条分支都钉住 |
| 79 | **报告开头的 p95 口径与实现不符**：写着"n=10 时 p95 实际就是第二大的那个样本"，而 `rank = ceil(0.95·10) = 10` → 是**最大的那个**（第二大的要 n=20） | 🟢 低（措辞） | 改写成实际规则并把两个样本量都写出来；测试同时钉住 `n=10 → 最大`、`n=20 → 第二大` |
| 80 | **明细里把字节当毫秒打印**：首屏 JS 的样本值是字节，`_fmt_ms` 把它印成"最小 130048 ms"（表格列里是"127.0 KB"，同一个数字两种单位） | 🟢 低（读数） | 按 key 选单位（`127.0 KB`），拿不到时写 `—` 而不是 `0` |
| 81 | **报告生成器里写死了一个会过期的数字**：`extra` 里那句"首页 LCP 已 **2.7s** 说明 TTI 多半越过 3s"，2.7s 是第一次运行的读数 —— 之后每次重跑都带着它（句子读起来很顺，所以没人发现）；而 `TTI ≥ LCP` 只给**下界**，推不出达标也推不出越线 | 🟢 低（诚实性） | 改为引用本次实测样本，并如实说明它只是下界、要判定必须做长任务分析 |
| 82 | **bash 里 `$PERF_PORT）` 会被吞掉括号的首字节**：变量紧跟全角标点时，bash 把那个字节读进变量名，报出来是一句看不懂的 `PERF_PORT<乱码>: unbound variable`（采集脚本**第一次运行就挂在**这句上） | 🟢 低 | 所有紧跟全角标点的变量写 `${VAR}`；这条坑写进脚本头，避免下次再踩 |
| 83 | **TTI 只收长任务会得到系统性偏乐观的数**：页面刚画完首帧、脚本还没开始跑的那一瞬间主线程往往是安静的，于是 TTI 会被判成 FCP。Lighthouse 用「静默窗内在途请求 ≤ 2」挡这种假静默，而第一版采集没收请求区间 | 🟡 中（诚实性：数字偏低且说不出低在哪） | 采集侧同时收 Resource Timing 的 `startTime → responseEnd`；判定侧把「≤2 个在途」作为窗口的**必须条件**，并用「8500ms 才卡 300ms」这个反例钉住它现在必须返回 `未测` |
| 84 | **门槛量成了「机器当时有多忙」**：束搜索那条在整套 `make check` 跑到时三次读数是 `[223.6, 326.17, 500.59]` ms（同一条用例单跑是 54/139/82ms），**501ms 只差 0.59ms** 就越过 500ms —— 而算法一个字节都没改。同一台机器上还跑着开发前后端与浏览器，CPU 噪声是**单向**的（只会更慢） | 🟡 中（会训练人「红了就重跑」） | 每种 archetype 重复 3 次取**最好一次**（`best_and_worst()`，纯函数 + 单测），门槛量的回到算法成本；**最差一次照样写进报告**（括号里），读者能自己看出当时机器有多忙 |
| 85 | **“没测到的东西”会被下游顺手编上一个**：TTI 这一项在报告里长期是 `⏭ 不可测`，而 `extra` 里那句「首页 LCP 已 2.7s，TTI 多半越过 3s」其实是从**一次**读数推的（2.7s 那次是 Hero 视频帧的 LCP，而 TTI 与 LCP 量的根本不是一回事：实测 TTI 569ms < LCP 中位 2592ms，因为视频继续画帧并不阻塞主线程） | 🟢 低（诚实性：措辞） | 报告里改成不给推论、只给两个各自的口径；并把「TTI 是主线程可交互，不是画完」写进 TTI 那一行的说明 |

### 这一轮的两条方法上的体会

1. **"文件头承诺的东西不存在"要和代码一样被查**：`benchmark.py` 的文件头说"分位函数与那条集成用例共用，口径不会漂"，
   而那个文件根本不在仓库里 —— 报告里那句"生成方式：`make perf`"也一样（Makefile 里没有这个目标）。
   这类**自我描述的漂移**不会让任何测试变红，只能靠"写文档时对着命令真跑一遍"发现（本轮就是给 perf 做收尾时才发现的）。
2. **门槛必须有"不许放宽"的一侧**：测试如果直接引脚本里的常量，把 8s 改成 30s 也一样绿（自指闸门）。
   所以 PRD 数值在测试里**按原文写死**，再与脚本常量互相钉住 —— 改任何一边都会红。
   与“没有断言的地方一定会漂移”同一条老规矩，只是这次漂移的是一种零成本的自我说服。

### 本轮门槛

| 项 | 结果 |
| --- | --- |
| 后端测试 | 1072 → **1095 passed**（+23：性能门槛 8→17 · 引擎缓存生命周期 3） |
| 后端覆盖率 | **95.61%**（闸门 93%） |
| 前端测试 / 覆盖率 | 394 passed（本轮只加了采集脚本，未改前端源码）；statements **99.06%** / branches 90.40% / functions 96.60% |
| 静态检查 | ruff 0 问题；mypy strict **126 文件** 0 错；`make check` 全绿（exit=0） |
| `make perf` 实跑 | 冷规划 p95 **202ms** · 缓存命中 242ms · 修改 259ms · 并发 20 全 200（5.3s）· 束搜索 53ms（最差一次 82ms）· 最慢单条 SQL 35ms · 分享页 LCP **392ms** · 首屏 JS **127 KB** · 首页 TTI **569ms** ✅ · 含搜索 **⏭ 不可测** |

### 刻意没做的事（如实记录）

- **TTI 不是 Lighthouse 分数**：没做它的像素级 LCP 候选回溯，也没有可访问性/SEO 审计；
  口径（5s 静默窗 + 在途请求 ≤ 2 + FCP/US 之后的最后一个长任务）与它对齐，但**不是**它的打分。
- **在途请求的区间只来自「已完成」的资源条目**：一直挂着不结束的请求（持续下载的媒体）
  在采集结束时是看不见的 —— 首页恰好有 Hero 视频，所以那个「≤2 在途」的条件在这一页是**部分覆盖**。
  要严格对齐 Lighthouse 得上 trace，这一条写进了报告，不用它给首页那个 569ms 加确定性。
- **不用 Lighthouse CLI**：要再下一份浏览器与一大包依赖；代价是**没有**可访问性/SEO 审计 ——
  报告里写明了这一点，不把它当 Lighthouse 分数用。
- **分享页 LCP 只测一条真实行程**（每次运行新造一条），不做多行程抽样：
  变化更大的是首屏资源而不是行程内容，样本量留给需要时再加。
- **不把 `make perf` 放进 `make check`**：它是分钟级、还要起前后端；
  进来就会变成没人愿意跑的门槛。秒级的那部分（冷规划/束搜索/报告口径）已经进了 `check`。
- **"含搜索"依旧不可测**：L8 未接进规划链（这是 M5 时记下的刻意决定）。
  脚本会先核实"搜索真的发生过"（读 `cost_logs.category='search'`），没发生就写"不可测"，
  不把 246ms 的普通规划当成"含搜索 p95"。

---

## 关系图报告的可归因性（2026-09-16）

> 触发自一件很具体的事：工作区里躺着一份未提交的 `docs/RELATION_REPORT.json`
> ——上一轮补完 M6 数据后重算关系图留下的，`osrm_pairs` 从 163 变成 161。
> 但报告里**只有这一个数**：读者无法分辨是"分批边界变了"还是"OSRM 失败了"。
> 所以处理方式不是"直接提交"，而是先让这份证据能解释自己。

| # | 问题 | 严重度 | 修法 |
| --- | --- | --- | --- |
| 83 | **OSRM 覆盖数不可归因**：`table` 接口一次只算一批（`batch_size: 25`）内部的矩阵，跳批次的候选对**必然**退回估算；而分批是按 UUID 排序切的，重跑建库会重新编号 → 分批边界跟着变。于是 `osrm_pairs` 的涨跌既可能是结构性的、也可能是失败批次造成的，**报告里两者不可分辨**。失败批次第一版只 `print` 一行，退出码仍是 0 | 🟡 中（证据不可信） | 报告新增 `osrm_eligible_pairs`（结构性上限）、`osrm_batches_ok/failed`、`batch_size` 与一句 `osrm_note`；顶部记录 `use_osrm` / `max_places` / `neighbors` / `generated_at` —— 一份既可能是"离线估算"又可能是"真实路网"的文件必须自己说明是哪一种（否则一次离线重算会静静覆盖掉带 OSRM 数字的报告，连痕迹都不留） |
| 84 | **`docs/DATA_REPORT.md` 里手写死的「本次 5445 对里 163 对」**：这份文档每次 `make seed` 都会重新生成，而这个数是手抄的 —— 它先是写着 M1 时代的关系图，后来被改成当前的对数、但真实路网对数没跟上（同一个错法在 `benchmark.py` 里也出现过，见 81） | 🟢 低（文档漂移） | 那段文案改为**只讲结构、不抄数字**：说明上限来自分批结构、估算值逐条带 `data_source`，具体对数/失败批次/生成时间全部指向 `docs/RELATION_REPORT.json`（一处出处）。它只读那份报告回答一件事：当前这份是真实路网还是离线估算模式（`use_osrm`），因为这决定了这段话该怎么读 |

### 四次重算的实测（同一份 5445 对候选）

| 次 | 何时产生 | 真实路网对数 | 批内可覆盖上限 | 失败批次 |
| --- | --- | --- | --- | --- |
| 1 | M1 时代（HEAD 里那份） | 163 | —（当时没记） | — |
| 2 | M6 补数据后（上一轮遗留） | 161 | —（当时没记） | — |
| 3 | 本轮重跑建库后 | 146 | **146** | 0 |
| 4 | 修好文案后的最终重跑 | **130** | **130** | 0 |

结论有两半，都要写下来：

1. **163 → 161 既不是回归、也不是网络失败**：列 3 与列 4 在最近两次里**逐对相等**
   （批内能拿到的都拿到了，`osrm_batches_failed = 0`）—— 变的只有分批边界。
2. **覆盖率会随机漂**：同一份候选、同一台机器，只是重跑一次建库（地点重新编号
   → 分批顺序变），真实路网对数就从 161 掉到 130。所以任何**抄下来的**覆盖率数字
   都是过期副本 —— 除非它带着分母与“上限”一起被记下来。

### 顺带对齐的三份文档（已备份）

`kb_version` 里带日期（`gz-2026.09.16-1a12399485`，摘要部分仍是内容寻址、未变），
所以文档要与库对齐就得真的重跑一次建库。按 `RUNNING.md` §4 的流程执行：

```bash
pg_dump … > /tmp/tripdecider_dev_20260916_110505.sql   # 12MB，先留退路
cd backend && PYTHONPATH=. uv run python scripts/seed_guangzhou.py --force
make relations OSRM=1     # 建库会级联清掉 place_relations，必须重算
make validate             # 0 阻断项 · 2 告警（B3 交叉核验 17.1% · B6 营业时间 83.9% 未知）
make perf                 # 顺带刷新 PERF_REPORT（旧分享链接会随建库 404）
```

代价如实记：`--force` 删掉了开发库里 **443 条本地行程**（含 3 条分享链接，属压测与手工冒烟产物），
地点 UUID 全部重新编号 —— 备份在 `/tmp/tripdecider_dev_20260916_110505.sql`，可整库恢复。

### 可选提升（本轮**没做**，如实留在这里）

分批当时按 **UUID 排序**切，而候选对几乎全是 6km 内的邻居 —— 也就是"把地理上挨着的点
拆到了不同批"。改成**按地缘分批**（同一批尽量装相邻的点）不需要改 `batch_size`、
不改接口，预期能把批内对数提高一个量级，是提升真实路网占比最便宜的一招。
本轮不动它：它是算法口径的改动（会影响 A/B 对比与缓存），不该混在"处理一份未提交的报告"里做。

> ✅ **已于 2026-09-16 补做**（同一轮数据真实性修复里，B4）：预期"提高一个量级"成立 ——
> 130 → 4085 对。动手的是同样的约束（不改接口、不动 `batch_size`）。

---

## 数据真实性修复（2026-09-16）：手动试用后的五项反馈

> 触发方式：不是测试报错，而是**人手动跑一条真实行程**（题面「广州 · 09:00–21:00」、
> 节奏 relaxed、预算 ¥400/人），把方案 A「老字号早茶慢行」的每个数字摊开对了一遍。
> 下面五项是使用者当时勾出来的"看起来不真实"，**它们全都是预测值，也都不是 bug** ——
> 而是"我们输出了一件事、但我们并不知道它"。

| # | 使用者看到的现象 | 严重度 | 根因与修法 |
| --- | --- | --- | --- |
| 85 | **预算低得离谱**：三站（点都德 / 光明广场 / 陶陶居）报 **¥19.59/人**，其中两顿老字号餐饮贡献 **¥0** | 🔴 高（假数） | 全库 3166 地点 **0 条有价格**（B5）。旧逻辑：餐饮站点"有但是没价" → 整笔计进 `unknown_items`（不计入金额）。于是 19.59 恰好等于那段打车费，读者会以为这两顿饭不要钱。改为：有餐饮站点但无价 → 用 `limits.yaml` 的餐费单价（早茶 30 / 午餐 60）推定并标 `estimated`，**并新增 `estimated_items` 逐项写明"含估算"**；两者语义相反（"算进去了但单价是假设" vs "没算进去"），因此分开存储、前端分两行渲染 |
| 86 | **停留时长得不到节奏影响**：每站 90 分钟、总时长 286 分钟（约 4.8 小时），而题面要的是 relaxed | 🟡 中 | `pace` 此前**只**影响 `walking_caps_m`：同一题面选「轻松」/「紧凑」排出的站点与每站停留**完全一致**，差别只有步行上限 —— 那不是两种节奏，是同一趟行程换了个标签。改为在 `config/limits.yaml` 新增 `stay_scale_by_pace`（relaxed 1.3 / balanced 1.0 / packed 0.85），由 `stay_duration_min(place, pace, limits=...)` 应用，并对齐 5 分钟取整（`117` 这种"假精度"会让读者以为是实测值）；`min_stop_duration_min` 仍是下限，缺配置时退回 1.0 而不是崩 |
| 87 | **营业时间全"未知"**：3 个站点全是「营业时间未知」（库里 `has_hours = 0`） | 🟡 中（数据缺口） | 就是 **B6**：`opening_hours_raw` 原文一直在库里（509 条），只是没有解析器。补上 `app/domain/opening_hours.py` 并在读路径解析（见 B6 行）：267/509 = 52.5% 解得出，解不出的继续如实报未知。**不猜、不补全**：一个猜错的开放时间比"未知"更危险 |
| 88 | **通勤耗时/距离不真实**：`taxi 2918m → 11 分钟`、`walk 319m → 5 分钟`，`source = estimated` | 🟡 中 | 就是 **B4**：候选对里只有 2.4% 能拿到真实路网距离，其余是"直线距离 × 绕路系数"。修好后同一对地点：光明广场 ↔ 点都德 从 **319m（直线推算）→ 760m（真实路网）**，`data_source = osrm`。注意口径：公共 OSRM 只有 car profile，`walk` 的**耗时**仍由真实距离 × 文档化速度推导并标 estimated —— 距离变真、时间仍然诚实标注，不会互相冒充 |
| 89 | **推荐理由空泛**：三个站点的 `why_recommended` 都是「美食」；`tips` 为空 | 🟢 低（文案） | 旧实现把**用户自己勾的偏好**当理由复述回去：同一趟里两座茶楼拿到一模一样的徽章，读者既无法核验也无法区分。改为 `_stop_why` 带上**分值**（库里 `food_score`，可回溯到 enrichment 的 tag_signals）与**地点自己的标签**（滤掉与维度标签重复的词），按分值降序取前两位 ⇒ 陶陶居 = `美食 0.90 · 早茶/老字号`、点都德 = `美食 0.90 · 早茶` |

### 一条贯穿五项的共同教训

五项里有四项的根因是同一个句式：**"字段存在"被当成了"字段有值"**。
`price_min` 列在、但 0 行有值；`pace` 字段在、但不影响任何一站；
`opening_hours_raw` 在、但没有解析器；`relations` 表在、但 97.6% 是估算。
测试全都绿，因为测试问的是"接口有没有返回"，而这些项问的是**"这个数是从哪来的"**。
所以这一轮的测试也都是这么写的：不测"字段非空"，而是测"没数据时它必须说不知道"、
"改了节奏它必须跟着变"、"分批后同批对数必须高于按 UUID 切块"（含一条反向对照）。

### 本轮验证

```bash
make check     # 后端 1143 passed · 覆盖率 95.82% · 前端 394 · statements 99.06% · ruff/mypy/tsc 全绿
make relations OSRM=1   # 5445 对 → osrm 4085 / estimated 1360 · 32 批 · 0 失败 · 28.3s
```
...
| 文件 | 条数 | 守什么 |
| --- | --- | --- |
| `tests/unit/test_budget.py` | 6 | 餐饮站点缺价时走配置估算（不再归零），且 `estimated_items` 与 `unknown_items` 不混 |
| `tests/unit/test_opening_hours.py` | 21 | 解析器边界：跨零点、`06:06-24:08` 这类脏数据、`Mo-Su 10:00-22:00`… **看不懂必须返回 None** |
| `tests/unit/test_candidates.py` | 6 | 节奏真的改停留（relaxed > balanced > packed）、5 分钟取整、不低于下限、缺配置退回 1.0 |
| `tests/unit/test_plan_service.py` | +7 | 推荐理由带分值、能区分两座同类茶楼、小于阈值时不给理由、徽章不超两位 |
| `tests/unit/test_compute_relations.py` | +7 | 分批覆盖率 ≥90%（带一条按 UUID 切块的反向对照）、不超 `batch_size`、可复现、孤立点按地理序补位 |

两条写测试时顺带查出来的事实（都已写进上面的行）：全库 **0/3166** 条有价格；
`opening_hours_raw` 有值 **509/3166** 条，解得出 **267 条（52.5%）**。

---

## 多日行程与「每天玩多久」（2026-09-16）：`days` 其实从没参与过规划

> 触发方式：使用者手动试用后问了一句"选两天玩三天，给出的方案总时长很短"。
> 查下去发现两件事，都不是 bug，是**没接上**。

| # | 问题 | 严重度 | 根因与修法 |
| --- | --- | --- | --- |
| 90 | **`days` 只进了标题**：选 2 天/3 天拿到的其实是**同一天**的行程，只是版权写了"2 天行程" | 🔴 高（内容与标题不一致） | `days` 被解析、校验（表单 1–3 天）、存库、写进标题与分享页，但**规划器从不读它** —— `plan_routes` 只有**一个**时间窗。修法：按天顺序组合，每天一份独立行程（`Stop.day`），第 N 天的候选池排除前 N-1 天用过的地点；总时长 = 各天之和（不是跨天跨度，那会把一夜睡眠算成游玩） |
| 91 | **一天也填不满窗口**：12 小时的窗口只排 4–5 小时 | 🟡 中 | 真正的限制不是时间，是 `scoring.yaml` 的 `max_stops`（relaxed **4 站** / themed 5 / classic 6，`planner.py` 直接 `max_depth = min(max_stops, 8)`）。修法：时间窗成为主依据，`max_stops` 抬高一档并回归"别跑飞"的安全阀；另新增 `day_span`（半天/一天/尽可能多）作为显式输入 |
| 92 | **「本地人只想玩个半天」没有入口** | 🟡 中 | 只能手动把结束时间改到中午，或改 pace（但"轻松"与"只玩半天"是两回事）。修法：`Intent.day_span` + 表单控件 + 规则引擎（"玩半天"/"一整天"/"尽可能多"），半天/一天的分钟数落在 `limits.yaml` 的 `day_span_minutes` |
| 93 | **时长只是"最晚到达时刻"**：半天档（4 小时）实测排出了 **6.5 小时** | 🟡 中 | 束搜索只查 `arrive > end_min`，再把整段停留加上去 —— 排到 3.5 小时的第 4 站可以坦然待 2 小时。修法：改判 `arrive + stay > end_min`（**这一站必须能在配额内结束**），并把半天档从 240 调到 **300** 分钟：按 90 分钟停留算，4 小时只够 2 站，半天档会一套方案都排不出来 |
| 94 | **多日把餐费算少了**：两天只收一顿午饭的钱 | 🟡 中 | 预算把整条站点序列当**一段连续时间**（`stops[0].arrive_min` → `stops[-1].depart_min`），两天就成了 30 小时的"一段"，每个餐次只计一次。修法：`estimate_route_budget` 按天拆开各算一次再相加 |
| 95 | **「需要留意」把同一句话列三遍**（E2E-14 红起来的就是它） | 🟡 中 | 营业时间类提示是**按站点**逐条产生的，文案里不带站名（哪一站的信息在 `detail.place` 里），所以三个站点都没填出行日期时就是三条**一字不差**的句子；前端列表以句子为 key，于是 React 报 duplicate key。修法：`_cons` 先按顺序去重再取 3 条（`tests/unit/test_plan_service.py` 钉住）。它是在 `opening_hours` 解析器上线之后才出现的 —— 那之前每句话里都带着**互不相同**的店名 |

### 实测（同一份真实知识库，空的偏好、预算 ¥2000/人）

| 请求 | 结果 |
| --- | --- |
| `days=1` + 一天档 | A 3 站 / 6.5h · B 5 站 / 8.9h · C 3 站 / 6.0h |
| `days=1` + 尽可能多 | A 3 站 / 6.5h · B 5 站 / 9.2h · C 5 站 / 9.9h |
| `days=1` + 半天档 | A/B 3 站 / **4.5h**（≤ 5 小时配额） · C 3 站 / 4.6h |
| `days=2` + 一天档 | A **2 天 6 站 / 12.6h**（391+364 分钟）· B 8 站 / 15.0h · C 6 站 / 12.1h |
| 修复前（用户实际看到的） | 3 站 / **4.8h**，且"2 天行程"只有一天 |

### E2E（2026-09-16 第一次真跑，不是"写在 Makefile 里"）

`make e2e`：**19 passed / 52.8s**。它当场找出两条问题，都已修：

1. **`E2E-03` 点不动「半天」**：Playwright 的 `.check()` 去点的是 radio 的**真身** ——
   `sr-only` 的 1px input，上面盖着 `<label>`，于是报 `intercepts pointer events`。
   它此前一直没暴露，是因为同一段里另外两个 `.check()` 选的值恰好是**默认值**：
   `check()` 对已选中的元素是空操作，压根不点。修法：`e2e/helpers.ts` 的 `chooseSegment()`
   改点**可见的胶囊**（用户真实做的动作），三处一起换。
2. 上表第 95 条（重复的「需要留意」）。

> ⚠️ 一个操作上的坑：`plan_cache` 的键是 `(params_hash, kb_version)`，**不含代码版本**，
> 所以改完规划输出后旧缓存会继续被复用（第一次跑 E2E 就是这么红的）。重跑前清一次：
> `psql -d tripdecider_dev -c "DELETE FROM plan_cache"`。
> `make e2e` 只清限流计数器 —— 它**故意**复用缓存，否则每 IP 每天 5 次冷规划会被 19 条用例烧穿
> （见 `playwright.config.ts` 与 `e2e/helpers.ts` 的说明）。

对照的一条观察（如实记）：`relaxed` 在"尽可能多"下只排 6.5 小时，仍是三套里最短的 ——
它的 `walking_cap_ratio: 0.80` 与 `walking_fit` 权重（0.24）主动压站数。
它不是错（A 的定位就是"轻松休闲"，与 B/C 拉开差别正是 PRD AC-6.3 要的），
但"轻松"与"想多去几个"同时出现时，用户可能在 A 里看不到自己想要的那个版本。
真要解决得引入"时长偏好参与评分"，本轮不动。

### 刻意没做的事（如实记录）

1. **LLM 叙事没区分"第几天"**：`RouteNarrativeInput` 只拿到一串站名（不传数字是刻意的），
   多日时模型不知道哪天是哪天。文案模板与数字全部由代码算，所以不影响正确性 ——
   本轮不为了文案去改 prompt。
2. **PRD 里没有多日语义** —— 已在 `PRD.md` 补 **FR-00**（每天玩多久 × 玩几天、
   跨天不重复、总时长为各天之和、餐费按天重复等 8 条），因为这不是"实现细节"：
...
| M6 | ~~补齐 B1/B2 数据缺口~~、关系图扩量、知识库扩至全类别达标 | B1/B2 **✅（2026-09-15）**；关系图扩量 **✅（2026-09-16）**：800 地点 / 5445 对，真实路网 **4085 对（75.0%）**，剩余 25% 是分批尺寸的固有上限（见 B4） |
| M7 | ~~E2E（Playwright 16 场景）、异常注入 25 项、性能与安全检查（含 p95）~~ | **✅ 全部完成（2026-09-16）** |
| M8 | 交付报告（PRD 第四十节 12 小节） | M0–M7 ✅ |
