...
| M7-5 | `tests/integration/test_perf_budget.py`（秒级门槛回归） | ✅ | 8 条：门槛常量 ↔ PRD 互相钉住、分位口径、冷规划 p95 ≤ 8s、束搜索 ≤ 500ms、**报告不说假话**（3 条） |
| M7-6 | `tests/unit/test_db_session.py`（引擎缓存生命周期） | ✅ | 3 条：缓存必被清空、跨循环关闭失败不允许留下毒丸、其余异常不许吞 |
| M7-7 | 报告重新生成 | ✅ | `docs/PERF_REPORT.md`：分享页 LCP **392 ms** ✅ · 首屏 JS **127 KB** ✅ · 首页 TTI **⏭ 未测** · 含搜索 **⏭ 不可测** |
...
| 项 | 结果 |
| --- | --- |
| 后端测试 | 1072 → **1083 passed**（+11：性能门槛 8 · 引擎缓存生命周期 3） |
| 后端覆盖率 | **95.61%**（闸门 93%） |
...
| `make perf` 实跑 | 冷规划 p95 **202ms** · 缓存命中 242ms · 修改 259ms · 并发 20 全 200（5.3s）· 束搜索 53ms · 最慢单条 SQL 35ms · 分享页 LCP **392ms** · 首屏 JS **127 KB** · 首页 TTI **⏭ 未测** · 含搜索 **⏭ 不可测** |

### 刻意没做的事（如实记录）

- **首页 TTI 仍然没测**：它的正确定义要长任务分析（首个 5s 静默窗之前最后一个长任务结束），
  拿 `domInteractive` 之类顶替只会得到一个偏低且含义不同的数 —— 报告里写"未测"并把 LCP
  的读数标成"TTI 的下界"（首页 LCP 中位 2.4s，仍在 3s 门槛内，但不能据此说 TTI 达标）。
