...
| B4 | 真实路网关系覆盖仅约 2.4% | 🟡 已定量 | 覆盖率的**上限是分批结构**：`table` 一次只算一批（`batch_size: 25`）内部的矩阵，跳批次的候选对必然退回估算。2026-09-16 实测：同一份 5445 对候选，四次重算的真实路网对数是 163 / 161 / 146 / **130**，**失败批次均为 0** —— 变的只是分批边界（按 UUID 排序切批，而重跑建库会重新编号）。要真提覆盖率：自建 OSRM、减小 `batch_size`（请求数上升）或**按地缘分批**（尚未做，见下节） |
...
分批目前按 **UUID 排序**切，而候选对几乎全是 6km 内的邻居 —— 也就是"把地理上挨着的点
...
| M6 | ~~补齐 B1/B2 数据缺口~~、关系图扩量、知识库扩至全类别达标 | B1/B2 **✅（2026-09-15）**；关系图扩量仍 ⏳（800 地点 / 5445 对，真实路网 130 对约 2.4% = 批内上限，见 B4） |
