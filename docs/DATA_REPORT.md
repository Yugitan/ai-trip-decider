...
- 生成时间：2026-09-16 03:06:44 UTC
- 知识库版本：`gz-2026.09.16-1a12399485`
- 原始 OSM 记录：**13067**
- 通过丰富化：**3241**
- 合并重复测绘：52
- 分店上限裁减：23
...
8. **关系图的真实路网占比有限，而且这个上限是结构性的**：OSRM 的 `table` 接口一次只算一批（`batch_size`）内部的矩阵，
   ...
   具体覆盖对数、失败批次数与生成时间见 `docs/RELATION_REPORT.json`（`make relations OSRM=1` 产出）—— 这份数据报告不抄那几个数字，因为它们会随重算变化，而抄来的数只会变成一处过期副本。
