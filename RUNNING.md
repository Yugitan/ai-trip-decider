...
### 8.14 行程时长：先分清"玩几天"与"每天玩多久"（2026-09-16）

结果页上跟"时间"有关的三个数来自三个不同的地方，调错了就白调：

| 想要的效果 | 改哪里 |
| --- | --- |
| 每天大概几点能结束（半天 / 一天 / 尽可能多） | `config/limits.yaml` 的 `planning.day_span_minutes`（`half_day: 300` / `full_day: 540`）；表单上叫「每天玩多久」 |
| 一天最多排几个地方 | `config/scoring.yaml` 各 archetype 的 `max_stops`（**节奏上限**，不是主依据） |
| 一趟能玩几天 | 表单的「天数」（1–3）；实际排出来的天数看 `trip.days`，用户请求的天数看 `intent_snapshot.days` |

两条容易踩的：

- **`day_span_minutes` 不能小于"3 站 × 典型停留"**。时长是**硬约束**（每站必须能在配额内结束），
  配额太小会直接排不出方案 —— 半天档曾定 240 分钟，按 90 分钟停留只够 2 站，
  低于 `min_route_stops: 3`，那一档等于不可用（现为 300）。
- **实际天数可能少于请求天数**（素材不够跨天不重复）。此时 `degraded_modes` 会出现
  `days_short:N/M`，标题写的是**实际**天数；修改行程的 Diff 比的则是**请求**天数
  （"改成 3 天"要能在文案里看到），两者故意分开。
