...
### FR-00 行程的时间语义（每天玩多久 × 玩几天） `[P0]`

> 补于 2026-09-16：原稿只规定"天数 1/2/3"，没规定**多日行程排成什么样**，
> 实现也真的没读它 —— 于是"2 天行程"的每一站都发生在同一天，而 09:00–21:00 的
> 12 小时窗口里只排了 4–5 小时（站数上限封顶，看不到时间窗还有多少空着）。
> 使用者手动试用时一眼就看出了不对。以下为补齐的语义。

**两个维度，不得混用**

- **`days`（玩几天）**：1–3。它决定行程**横跨几天**，不决定每天多长。
- **`day_span`（每天玩多久）**：`half_day`（半天，≈5 小时）/ `full_day`（一天，≈9 小时）/
  `whole_window`（用满用户给的时间窗）。它决定**每天排多少**，与天数无关。

**规则**

1. **每天一份独立行程**：多日行程按"第 1 天 / 第 2 天 …"分组渲染，每组有各自的起止时刻。
2. **每天同一套时间窗**：第 2 天的 09:00 就是 09:00，不跨天累加（不是"第 33 小时"）。
3. **跨天不重复**：同一个地点在同一条方案里只出现一次。
4. **时长是承诺**：一天的最后一站必须能在当天配额内结束（只查到达时刻会让"半天"排出 6.5 小时）。
5. **站数上限服从节奏**：`max_stops` 是"别跑飞"的上限，不是一天容量的主依据；
   主依据是时间窗（配额），到达节奏上限或时间窗上限，以先到者为准。
6. **总时长 = 各天之和**：不是"第一天早上到最后一天晚上"的跨度（那会把中间那夜的睡眠算成游玩时长）。
7. **餐费按天重复**：两天的行程吃两顿饭，不做"整段连续时间"的合并计算。
8. **实际天数可能少于请求**：素材不够时如实少排并标记降级原因，不拿重复地点凑数，
   也不在标题上假装排满。
...
| 天数 | 1 / 2 / 3 | ✅ | 1 | 整数 1–3 |
| 每天玩多久 | 半天 / 一天 / 尽可能多 | ✅ | 一天 | 枚举（`half_day` / `full_day` / `whole_window`） |
| 人数 | 数字 1–20 | ✅ | 2 | 整数 |
...
{
  "intent": {
    "city": "guangzhou", "days": 1, "day_span": "full_day", "people": 2,
